...
exports.ingestBusData = (req, res) => {
  NextVehicleArrivalSystem.request({route: '60'})
    ...
    .then(() => res.end('Sucess: Data Ingested'))
  ...
  if (req.query.route) {
    Model.BusLocation.all({where: {route: req.query.route}}).then(data => {
      if (data.length > 0) {
        ...
        res.render('index', {
          data: {title: 'No data returned from request'}
        })
      }
    }).catch(err => {
    ...
    console.log('case no params')
    res.render('index', {
      data: {title: 'Page requires route query param'}
    })
  }
}
...
exports.timedSampleIngest = () => {
  ...
  .then(() => console.log('Sample Ingest successful'))
  .catch((err) => console.warn('Sample Ingest failed', err))
}
