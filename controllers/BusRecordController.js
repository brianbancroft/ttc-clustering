...
exports.ingestBusData = (req, res) => {
  NextVehicleArrivalSystem.request()
    .then((data) => {
      ...
      data.vehicle.map((bus) => Model.BusLocation.create(BusLocationSetup.singleInstance(bus, refGeoJSON)))
    })
    .then(() => {
...
exports.timedSampleIngest = () => {
  ...
  .then((data) => {
    ...
    // gathers all points for clustering comparasion
    const refGeoJSON = GeoJSONConversion.setupPointCollection(data.vehicle)
    data.vehicle.map((bus) => Model.BusLocation.create(Model.BusLocationSetup.singleInstance(bus, refGeoJSON)))
  })
  .then(() => {
